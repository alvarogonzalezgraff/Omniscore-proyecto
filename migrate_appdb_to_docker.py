...
print('=== MIGRACIÓN DE APP.DB A DOCKER POSTGRES ===')
print()
...
# 4. Migración de tablas principales
...
tables_to_migrate = [
    ...
    ('teams', 'id', [
        'id', 'league_id', 'name', 'logo_path'
    ]),
    ('matches', 'id', [
        'id', 'league_id', 'home_team_id', 'away_team_id', 
        'matchday', 'match_date', 'home_score', 'away_score', 'is_finished'
    ]),
    ('goals', 'id', [
        'id', 'match_id', 'team_id', 'player_name', 'minute', 
        'assist_player_name', 'is_own_goal', 'is_penalty'
    ]),
    ('cards', 'id', [
        'id', 'match_id', 'team_id', 'player_name', 'minute', 
        'card_type', 'reason', 'description'
    ]),
    ('substitutions', 'id', [
        'id', 'match_id', 'team_id', 'player_in', 'player_out', 'minute'
    ]),
    ('injuries', 'id', [
        'id', 'match_id', 'team_id', 'player_name', 'minute', 'description'
    ]),
    ('penalties', 'id', [
        'id', 'match_id', 'team_id', 'player_name', 'minute', 
        'outcome', 'description'
    ...
    ('scraped_matches', 'id', [
        'id', 'league', 'season', 'matchday', 'date', 'home_team', 
        'away_team', 'home_score', 'away_score', 'is_finished', 
        'scorers', 'updated_at', 'assists', 'yellow_cards', 
        'red_cards', 'substitutions', 'injuries', 'url'
    ]),
    ('scraped_data', 'id', [
        'id', 'sport', 'league', 'season', 'team_name', 'position', 
        'points', 'matches_played', 'wins', 'draws', 'losses', 
        'goals_for', 'goals_against', 'goal_diff', 'form', 'updated_at'
    ]),
    ('player_stats', 'id', [
        'id', 'league_id', 'player_name', 'team_name', 'goals', 
        'assists', 'matches_played', 'updated_at'
    ])
]
...
for table_name, primary_key, columns in tables_to_migrate:
    ...
    try:
        ...
        # Limpiar tabla destino en PostgreSQL (opcional - comentar si quieres preservar datos)
        ...
        for row in rows:
            try:
                # Limpiar valores
                clean_row = [clean_value(value) for value in row]
...
if total_errors == 0:
    ...
    print(f'❌ Se encontraron {total_errors} errores durante la migración')
    print('🔍 Revisa los errores específicos arriba para más detalles')
